...
class kate6Service:
    ...
    def find_by_id(self, entity_id):
        ...
        data = kate6.query.get(entity_id)
        if data is None:
            raise kate6ServiceError(
                'kate6 {} not found'.format(entity_id),
    ...
    def get_all(self):
        list = []
        data = kate6.query.filter(
            kate6.tenantId == self.auth_context.tenant_id)
    ...
    def update(self, model, entity_id):
        ...
        record = kate6.query.get(entity_id)  # kate6 is a mongo class
        if record is None:
            raise kate6ServiceError(
                'kate6 {} not found'.format(entity_id),
                ErrorCodes.NOT_FOUND)
